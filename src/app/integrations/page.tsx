      ...
      <main className="relative z-10 mx-auto w-full max-w-3xl flex-1 px-5 py-10 md:px-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight text-slate-100">Integrations</h1>
              <p className="text-sm text-slate-400">Hubungkan Task Tracker ke aplikasi eksternal Anda</p>
            </div>
          </div>
        </div>

        {/* Architecture notice */}
        <div className="mb-6 flex items-start gap-3 rounded-xl border border-amber-500/20 bg-amber-500/10 px-4 py-3.5">
          <span className="mt-0.5 text-amber-400">🔗</span>
          <div>
            <p className="text-xs font-semibold text-amber-300">Bridge Architecture</p>
            <p className="mt-0.5 text-xs leading-relaxed text-amber-400/70">
              Semua komunikasi ke Odoo dikelola oleh <code className="rounded bg-amber-500/20 px-1 text-amber-300">odooService.ts</code>.
        ...
        <section>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {COMING_SOON.map((item) => (
              <div
                key={item.name}
                className="flex items-center gap-3.5 rounded-xl border border-white/[0.06] bg-slate-900/60 px-4 py-3.5 opacity-50"
              >
                ...
                <span className="ml-auto rounded-full bg-slate-700/50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-slate-500">
                  Soon
                </span>
              </div>
            ))}
        ...
        {/* Data flow diagram */}
        <section className="mt-10 rounded-2xl border border-white/[0.06] bg-slate-900/60 p-6">
          <h2 className="mb-4 text-sm font-semibold text-slate-300">Alur Data Sinkronisasi</h2>
          <div className="flex flex-wrap items-center justify-center gap-2 text-center text-xs">
            {[
              ...
              item === null ? (
                <svg key={i} width="24" height="16" viewBox="0 0 24 16" fill="none" className="text-slate-600">
